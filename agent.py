...
if not USE_MEMGPT:
    ...
    coder = create_memgpt_autogen_agent_from_config(
        ...
        f"a product manager ({pm.name})"
        f"and a code reviewer (MemGPT_critic).",
    ...
    critic = create_memgpt_autogen_agent_from_config(
        "MemGPT_critic",
        llm_config=llm_config_memgpt,
        system_message=f"I am a critic specialised in code review and requiremend checking, trained in Python and language. You review all code and requirements. You check all documents that they include what is required.\n"
        f"You are participating in a group chat with a user ({user_proxy.name}) "
        f"a product manager ({pm.name})."
        f"and a coder  ({coder.name}).",
        interface_kwargs=interface_kwargs,
        # Set a default auto-reply message here (non-empty auto-reply is required for LM Studio)
        default_auto_reply="...",
    )
...
groupchat = autogen.GroupChat(
    agents=[user_proxy, pm, coder, critic], messages=[], max_round=12)
manager = autogen.GroupChatManager(groupchat=groupchat, llm_config=llm_config)

# Begin the group chat with a message from the user
user_proxy.initiate_chat(
    manager,
    message="I want a pong game.",
)
